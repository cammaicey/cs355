...
class WireframeViewer(wf.WireframeGroup):
            ...
            if self.displayFaces:
                for (face, colour) in wireframe.sortedFaces():
                    ...
                    if towards_us > 0:
                        ...
                        #Use the Phong model
                        L = self.light_vector/np.linalg.norm(self.light_vector)
                        N = normal
                        V = self.view_vector/np.linalg.norm(self.view_vector)

                        ka = 0.1  # Ambient coefficient
                        kd = 0.2  # Diffuse coefficient
                        ...
                        dotLN = np.dot(L, N)
                        if dotLN > 0:
                            diffuse = self.light_color * (kd * dotLN * colour)

                            #Reflection
                            R = 2 * dotLN * N - L
                            R /= np.linalg.norm(2 * dotLN * N - L)

                            #Specular
                            dotVR = np.dot(V, R)
                            if dotVR > 0:
                                specular = self.light_color * (ks * (dotVR ** kgls) * colour)
                            else:
                        ...
                        light_total = light_total.astype(int)
    ...
    def keyEvent(self, key):
        ...
        if key == pygame.K_w:
            ...
            self.light_vector = rotX(self.light_vector, -step)
        if key == pygame.K_a:
            self.light_vector = rotY(self.light_vector, +step)
        if key == pygame.K_s:
            self.light_vector = rotX(self.light_vector, +step)
        if key == pygame.K_d:
            self.light_vector = rotY(self.light_vector, -step)
        if key == pygame.K_q:
            self.light_vector = rotZ(self.light_vector, -step)
        if key == pygame.K_e:
            self.light_vector = rotZ(self.light_vector, +step)
        self.light_vector = self.light_vector / np.linalg.norm(self.light_vector)
